...
namespace DB_Kurs.Формы
{
  public partial class Main
  {
    ...
    private void OperationsDeleteClick(object sender, EventArgs e)
    {
      var row = db.operations.Where(x => x.id.ToString() == operations_id_tb.Text.ToString()).First();
      db.operations.Remove(row);
      db.SaveChanges();
      operations_datagrid.Rows.RemoveAt(this.row);
      this.row--;
      if (this.row < 0 && operations_datagrid.RowCount > 0)
        this.row = 0;
      for (int i = 0; i < tmap_datagrid.RowCount; i++)
        if (tmap_datagrid[tmap_operation_id.Index, i].Value.ToString() == operations_id_tb.Text)
          tmap_datagrid.Rows.RemoveAt(i);
      if (tmap_id_operation_cb.Items.Contains(operations_id_tb.Text))
        tmap_id_operation_cb.Items.Remove(tmap_id_operation_cb.Items.IndexOf(operations_id_tb.Text));
      OperationsCellClick(this, new DataGridViewCellEventArgs(operations_id.Index, this.row));
    }
    private void OperationsSaveClick(object sender, EventArgs e)
    {
      try
      {
        var operations = db.operations.Where(x => x.id.ToString() == operations_id_tb.Text).First();
        ...
        operations_datagrid[operations_setup_time.Index, row].Value = operations.setup_time;
        operations_datagrid[operations_pg_time.Index, row].Value = operations.processing_time;
        operations_datagrid[operations_transit_time.Index, row].Value = operations.transit_time;
        operations_datagrid[operations_description.Index, row].Value = operations.description;
        operations_datagrid[operations_next_id.Index, row].Value = operations.next_operation_id;
        operations_datagrid[operations_id_wc.Index, row].Value = operations.work_center_id;
      }
      catch (Exception ex)
      {
    ...
    private void OperationsCellClick(object sender, DataGridViewCellEventArgs e)
    {
      if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && e.RowIndex < operations_datagrid.RowCount)
      {
        ...
        operations_wc_id_cb.Items.IndexOf(operations_datagrid[operations_id_wc.Index, row].Value);
      ...
      operations_save_btn.Enabled = true;
    }
  }
}
